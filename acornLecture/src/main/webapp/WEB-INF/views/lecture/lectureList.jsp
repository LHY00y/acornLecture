...
<body>
		...
		<c:forEach var="lecture" items="${lecturesList}">
			...
			<tr align="center">
				...
				<td title="${lecture.lecture_content }"><a href="${ contextPath }/lecture/info.do?id=${lecture.lecture_id}">${lecture.lecture_title }</a></td>				
				<td>${lecture.professor_name }</td> 
 				<td>${lecture.schedule }</td>
 				<td>${lecture.stu_count} &#47; ${lecture.count }</td>
			</tr>
			</c:if>
		</c:forEach>
	</table>
